...
//"define all \"wire\" references to \"wire1\" "
//#DEFINE wire wire1
...
//use i2c_t3 instead of wire.h because we're using the teensy SCL1 and SDA1 ports for i2c communication
#include <i2c_t3.h>
...
void Adafruit_LSM303_Accel_Unified::read()
{
  ...
  Wire1.beginTransmission((byte)LSM303_ADDRESS_ACCEL);
  #if ARDUINO >= 100
    Wire1.write(LSM303_REGISTER_ACCEL_OUT_X_L_A | 0x80);
  #else
    Wire1.send(LSM303_REGISTER_ACCEL_OUT_X_L_A | 0x80);
  #endif
  Wire1.endTransmission();
  Wire1.requestFrom((byte)LSM303_ADDRESS_ACCEL, (byte)6);
